...
class ImageTransformerDenoiserModelV2(nn.Module):
    ...
    def forward(self, x, sigma, aug_cond=None, class_cond=None, mapping_cond=None, input_image_condition=None):
        assert class_cond is None and mapping_cond is None
        if input_image_condition is not None:
            x = torch.cat([x, input_image_condition], dim=1)
        # Patching
        x = x.movedim(-3, -1)
        x = self.patch_in(x)
