...
means    = torch.tensor([[0., 1.], [0., 1.]])
b_scales = torch.tensor([[0.3, 0.1], [0.3, 0.1]])
weights  = torch.tensor([0.30, 0.70])

pX = D.MixtureSameFamily(
    D.Categorical(weights),
    D.Independent(D.Laplace(loc=means, scale=b_scales), 1)
)

def _log_normlaplace_1d(y, mu, b, sigma):
    z  = y - mu
    t1 = ( z / b) + botorch.utils.probability.utils.log_ndtr(-(z / sigma) - (sigma / b))
    t2 = (-z / b) + botorch.utils.probability.utils.log_ndtr( (z / sigma) - (sigma / b))
...
def grad_log_pY(y, gamma):
    y = y.clone().detach().requires_grad_(True)
    lp = log_pY_given_gamma(y, gamma)
    (g,) = torch.autograd.grad(lp.sum(), y)
    return g
...
def score_diff_y(y, x, gamma):
    if float(gamma) == 0.0:
        ...
        return x - prior_mean
    return -(y - gamma * x) / gamma - grad_log_pY(y, gamma)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        log_probs_pX = pX.log_prob(grid_points).reshape(grid_size, grid_size).numpy()

    levels_bg = torch.linspace(log_probs_pX.min(), log_probs_pX.max(), 20)

    fig, ax = make_ax(width, height)
    ax.contourf(xx, yy, log_probs_pX, levels=levels_bg, cmap=trunc_cmap, linewidths=0.0, antialiased=True)
    vmin, vmax = qv.sqrt().min().item(), qv.sqrt().max().item()
    cf = ax.contour(xx, yy, qv.sqrt().numpy(), levels=np.linspace(vmin, vmax, 15),
                    cmap=trunc_cmap2, vmin=vmin, vmax=vmax, linewidths=0.3)
    for txt in ax.clabel(cf, fmt="%.1f", fontsize=4, inline=True, inline_spacing=0.5, manual=False):
        txt.set_clip_path(ax.patch)
    ax.scatter(x1_sample[0,0], x1_sample[0,1], s=50, color='white', marker='*',
               label=r'$\bm{x}_{\text{ref.}}$', edgecolor='black', linewidth=0.5, zorder=10)
    plt.tight_layout()
    plt.subplots_adjust(left=0, right=1, bottom=0, top=1)
    plt.savefig('laplace_global.pdf')
